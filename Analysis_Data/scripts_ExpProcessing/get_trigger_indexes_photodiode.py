...
def signal_difference(sig1,sig2,doflip):
    if doflip==True: newsig=sig1-sig2
    else: newsig=sig2-sig1
    return newsig


def get_trigger_indexes_photodiode(data_raw_file='PAT_3066_EEG_708977_Anon_CategoryLocalizer.edf',data_path = '/Volumes/GoogleDrive/My Drive/EEG_DATA_PIERRE/PAT_3066',trigger_channel='Xe1',t_start = 0,t_end=-1,threshold_val=0.8,do_plot=True,t_shift=35, flip_trigs=False,skip_samples=50, binary_file='0'):
    """
    ...
    if filetype=='edf': 
        ...
        if binary_file != '0': 
            picks = mne.pick_channels_regexp(raw.info['ch_names'], regexp=binary_file)
            if t_end == -1: t_end = raw.__len__()
            sig2 = raw.get_data(picks=picks,start=t_start,stop=t_end)
        ...
        ## THIS TRC READ DOESNT WORK FOR NOW
        # Load the .TRC file
        reader = neo.rawio.WinWcpRawIO(filename=data_raw_file)
        reader.parse_header()
        # Extract the signal
        raw = reader.get_analogsignal_chunk()[0]
    else:
        print('SELECT APPROPRIATE FILE TYPE')
        stop
    
    # If peeks are opposide values (i.e. flipped on y = 0:
    if binary_file != '0':
        raw = signal_difference(raw,sig2,flip_trigs)
    elif flip_trigs==True: raw = raw*-1
    ...
    if do_plot == True:
        ...
        plt.title("offset = 0, onset = x")
        plt.plot(time_s,raw)
        plt.scatter(index_final_offset-t_shift,raw[index_final_offset-t_shift],marker='o',c='red')
        plt.scatter(index_final_onset-t_shift,raw[index_final_onset-t_shift],marker='x',c='green')
        plt.show()
